...
# S3 Config
# Load from environment variables. No hardcoded defaults for security.
...
def upload_to_s3(file_path, object_name=None):
    ...
    if not all([S3_ENDPOINT_URL, S3_ACCESS_KEY_ID, S3_SECRET_ACCESS_KEY, S3_BUCKET_NAME]):
        print("S3 configuration missing. Skipping S3 upload.")
        return None

    try:
        # Configure Boto3 to disable SSL verification if needed or use specific config
        config = Config(
        ...
        s3_client = boto3.client(
            ...
            region_name='auto',
            config=config,
            verify=False # Disable SSL verification to avoid environment certificate issues
        )

        # Suppress InsecureRequestWarning if verifying is disabled
        import urllib3
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

        s3_client.upload_file(file_path, S3_BUCKET_NAME, object_name)
        # Handle cases where endpoint might have trailing slash
        endpoint = S3_ENDPOINT_URL.rstrip('/')
        url = f"{endpoint}/{S3_BUCKET_NAME}/{object_name}"
